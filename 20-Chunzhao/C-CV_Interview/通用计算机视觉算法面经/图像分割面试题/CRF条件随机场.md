...
= \sum_{x} \overline{P}(x) log\sum_{y}exp(\sum_{k=1}^{K} \omega_k f_k(y, x)) - \sum_{x,y} \overline{P}(x, y) \sum_{k=1}^{K} \omega_k f_k(y, x)
$$
对w求导:
$$
\frac{\partial f(w)}{\partial \omega_k} = \sum_{x,y} \overline{P}(x, y) f_k(y, x) - \sum_{x,y} \overline{P}(x,y)f(x,y)
$$

## 维特比解码
维特比算法是一种动态规划算法，用于求解线性链条件随机场模型的最优路径。维特比算法的时间复杂度为O(nm^2)，其中n为序列长度，m为标记的取值个数。

<img src=https://s2.loli.net/2024/05/09/4JXawWRxUsLodBt.png width='100%'>

**linear-CRF模型维特比算法流程**

<img src=https://s2.loli.net/2024/05/09/2QwPr4JiYI1Rb63.png width='100%'>
