...
def channel_shuffle(x, groups):
    ...
    return x
```
> 解释上面代码: 首先将输入张量x进行reshape操作, 将通道数分成groups组, 然后进行转置操作, 最后再进行flatten操作, 将通道数重新排列。哪一步实现了shuffle? 通过transpose操作实现了通道之间的交换. 最终得到的x通道和之前相比: 通道之间的顺序发生了变化. 这个是随机的吗? 不是随机的, 是按照一定的规则进行的. 在哪一步发生了变化? 在transpose操作之后, 通过transpose操作将通道之间的顺序进行了交换. transpose的原理
