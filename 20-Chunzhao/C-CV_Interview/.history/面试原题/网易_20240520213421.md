...
- 为啥要对近邻数据做类别判断？
  - 对近邻数据进行类别判断通常是在执行基于近邻的分类或数据增强技术时进行的。例如，在SMOTE（Synthetic Minority Over-sampling Technique）算法中，对近邻的类别进行判断是为了只对少数类样本进行过采样。这样做可以帮助改善分类器在不平衡数据集上的性能，通过生成接近决策边界的合成样本来提高分类器的泛化能力。
- ResNet有什么优点？解决了什么问题？
