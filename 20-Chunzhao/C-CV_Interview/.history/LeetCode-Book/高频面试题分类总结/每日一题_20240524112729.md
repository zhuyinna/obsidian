...
> 贪心+单调栈
用单调栈的思想, 将数字越小的放在前面, 记栈中的元素数目为 m，我们不断地进行操作直到不满足条件：如果 m>0m > 0m>0 且 m+n−i>km + n - i > km+n−i>k 且单调栈的栈顶元素大于 nums[i]\textit{nums}[i]nums[i]，那么说明栈顶元素可以被当前数字 nums[i]\textit{nums}[i]nums[i] 替换，弹出单调栈的栈顶元素。
