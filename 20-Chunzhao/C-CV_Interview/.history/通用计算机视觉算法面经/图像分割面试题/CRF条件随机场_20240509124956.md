...
对w求导:
$$
\frac{\partial f(w)}{\partial \omega_k} = \sum_{x} \overline{P}(x) \frac{1}{\sum_{y}exp(\sum_{k=1}^{K} \omega_k f_k(y, x))} \sum_{y} exp(\sum_{k=1}^{K} \omega_k f_k(y, x)) f_k(y, x) - \sum_{x,y} \overline{P}(x, y) f_k(y, x)\
