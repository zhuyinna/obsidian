...
> 补充: 编码因子(分母): 当模型要处理高维特征, <img src=https://s2.loli.net/2024/05/07/VIUx4lJbcQwq1OR.png width='10%'> 增长越慢, 因为指数变小, 这种平滑的变化有助于模型在学习过程中不会对位置的微小变化过度敏感, 从而更好地泛化.
> 为什么用sin和cos? 官方解释:位置编码的每个维度都对应于一个正弦曲线。波长形成一个从2π到10000·2π的几何轨迹。我们之所以选择这个函数，是因为我们假设它可以让模型很容易地通过相对位置进行学习，因为对于任何固定的偏移量k,PEpos+k都可以表示为PEpos的线性函数
> 在偶数维度和奇数维度分别使用正弦和余弦? 保证不同维度的正交性, 独立表示, 能够让模型更好的学习到来自不同位置的特征.
