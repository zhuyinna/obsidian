...
什么是重参数? 重参数技巧是一种用于生成模型的技巧，主要是为了解决生成模型中的采样问题。生成模型的目的是生成一个与真实数据分布相似的数据分布，而生成模型的核心问题就是如何从一个分布中采样。目前的生成模型很多，主要包括如 $Autoencoder$,$VAE,GAN$ 为代表及其一系列变形的生成模型。我们今天主要聊聊 $VAE$~
