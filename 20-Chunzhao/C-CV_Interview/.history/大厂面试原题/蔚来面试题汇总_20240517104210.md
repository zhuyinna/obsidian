...
- BN为什么设置两个可学习参数
  - BN(Batch Normalization)是一种用于加速深度神经网络训练的技术, 通过对每个batch的数据进行归一化, 使得每层的输入数据分布稳定, 有利于梯度的传播和收敛. BN中的两个可学习参数: The two learnable parameters, γ (scale) and β (shift), are introduced in Batch Normalization to restore the representation power of the network. They allow the network to undo the normalization if it is found to be detrimental to the model's performance.
