...
- SGD和Adam
  ...
  - 举例来说, 训练是下山, 下山方向=优化方向=梯度/动量, 下山步长=学习率. SGD是固定步长下山, 每走一步需要环视一圈并找到最快的方向, Adam是自适应步长下山, 每走一步都会根据之前的走法调整步长, 以便更快地走到山底. 选择哪个优化器取决于具体问题，但Adam由于其稳定性和效率，在实践中非常受欢迎。
- BN的底层实现: BN的底层实现主要包括两部分，一是对输入数据进行归一化，二是对归一化后的数据进行缩放和平移
  - 归一化：对每个特征维度进行归一化，使得均值为0，方差为1
  - 缩放和平移：通过学习两个参数gamma和beta，对归一化后的数据进行缩放和平移，使得模型可以学习到适合的特征表示
  - BN的底层实现通常使用数学公式来描述，包括均值、方差、缩放和平移等操作，通过计算图和反向传播算法来实现
  - 在pytorch中通过`torch.nn.BatchNorm1d`或`torch.nn.BatchNorm2d`等模块来实现BN操作, 其中1d和2d区别在于输入数据的维度
- 通道concat实现: `torch.cat(**, dim=0)`