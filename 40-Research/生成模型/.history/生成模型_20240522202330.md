...
$$
其中第一项为预测的x0, 第二项为direction poiting to x_t, 第三项为噪声项, 噪声想中的$\epsilon_t$是从标准正态分布中采样的, 与x_t无关, 而$\epsilon_\theta^{(t)}(x_t)$是从网络中采样的, 两者是不同的, 但是由于网络的参数是固定的, 所以$\epsilon_\theta^{(t)}(x_t)$是确定的, 所以可以将两者合并成一个正态分布, 从而减少采样的随机变量. 如果$\sigma_t^2=\beta_t$, 采样过程和DDPM完全一样, 
