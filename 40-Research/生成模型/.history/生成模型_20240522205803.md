...
2. 加速采样
- 如何去马尔科夫化?
假设$P(x_{t-1}|x_t,x_0)\sim N(kx_0+mx_t,\sigma^2I)$, 满足以下两个方程:
$$
\begin{aligned}
x_t=&\sqrt{\alpha_t}(\frac{x_{t-1}-\sqrt{1-\alpha_t}\epsilon_\theta^{(t)}(x_{t-1})}{\sqrt{\alpha_t}})+\sqrt{1-\alpha_{t-1}-\sigma_t^2}\epsilon_\theta^{(t)}(x_{t-1})+\sigma_t\epsilon_t \\
x_{t-1}=&\sqrt{\alpha_{t-1}}(x_t-\sqrt{1-\alpha_t}\epsilon_\theta^{(t)}(x_t))+\sqrt{1-\alpha_t-\sigma_t^2}\epsilon_\theta^{(t)}(x_t)+\sigma_t\epsilon_t
\end{aligned}
$$
其中$\epsilon_t\sim N(0,I)$是噪声，$\epsilon_\theta^{(t)}(x_t)$是模型输出的噪声。
