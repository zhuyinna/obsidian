...
5. 缺陷
在对$p_\theta(x_{t-1}|x_t,x_0)$的推导中, 使用了一阶markov假设, 使得$p_\theta(x_{t-1}|x_t,x_0)=p_\theta(x_{t-1}|x_t)$, 因此重建步很长, 速度慢.

### DDIM

不再限制一阶markov, 使得重建步可以很短, 速度快
...
$$
其中第一项为预测的x0, 第二项为direction poiting to x_t, 第三项为噪声项, 噪声想中的$\epsilon_t$是从标准正态分布中采样的, 与x_t无关, 而$\epsilon_\theta^{(t)}(x_t)$是从网络中采样的, 两者是不同的, 但是由于网络的参数是固定的, 所以$\epsilon_\theta^{(t)}(x_t)$是确定的, 所以可以将两者合并成一个正态分布, 从而减少采样的随机变量. 如果$\sigma_t^2=\beta_t$, 采样过程和DDPM完全一样, 如果$\sigma_t^2=0$, 采样过程没有随机过程
