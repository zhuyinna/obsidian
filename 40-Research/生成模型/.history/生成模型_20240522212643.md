...
$$
其中用到了马尔可夫性质, 所以采样必须严格遵守马尔可夫, 一步一步来, 且为了保证xT是满足N(0,1), T也不能很小
...
- $\sigma$取值?
作者令$\sigma=\eta\sqrt{\frac{1-\overline{\alpha}_{t-1}}{1-\overline{\alpha}_t}}\beta_t$, 其中$\eta\in[0,1]$, 当$\eta=1$, 回到了DDPM, 当$\eta=0$, 为DDIM, 变成了确定性模型, 即每个xT对应一个确定的x0, 类似于GAN和VAE

- 由于DDIM是在DDPM上的扩展, 所以训练好的DDPM可以直接拿来通过DDIM的采样方法进行采样, 而不需要重新训练.
