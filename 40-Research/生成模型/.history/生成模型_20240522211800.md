...
将其中的t-1改为s, 得到:
$$
x_{s}=\sqrt{\overline{\alpha}_{s}}(\frac{x_k-\sqrt{1-\overline{\alpha}_s}\epsilon_\theta(x_k)}{\overline{\alpha}_s})+\sqrt{1-\overline{\alpha}_{s}-\sigma^2}\epsilon_\theta(x_t)+\sigma\epsilon
$$
