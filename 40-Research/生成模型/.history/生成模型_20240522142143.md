...
\end{aligned}   
$$
由于$\alpha_t^2+\beta_t^2=1$,那么 $(\alpha_1\dots\alpha_2)\beta_1\epsilon_1+\dots+\alpha_{t}\beta_{t-1}\epsilon_{t-1}+\beta_t\epsilon_t$即为多个独立正态噪声之和, 均值为0,  方差为 $(\alpha_1\dots\alpha_2)^2\beta_1^2$ , $$(\alpha_1\dots\alpha_3)^2\beta_2^2$, $\dots$, $\beta_t^2$, 叠加这些正态分布, 最终得到的是均值为0,  
